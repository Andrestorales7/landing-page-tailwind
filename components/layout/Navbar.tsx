...
const Navbar: React.FC = () => {
  ...
  const toggleProductMenu = () => {
    ...
    if (isProductMenuOpen) setActiveCategory(null);
  };
  ...
  useEffect(() => {
    ...
    const categoryTimeoutId = categoryTimeoutRef.current;
  ...
  const scrollToSection = (sectionId: string) => {
    ...
    if (router.pathname !== "/") {
      router.push(`/#${sectionId}`).then(scrollToElement);
    } else {
      scrollToElement();
    }
  };

  // Product categories estáticas
  const productCategories = [
    ...
    },
    {
      ...
      subcategories: [
        ...
        { name: "Ver todos", id: "soluciones", path: "/productos" },
      ],
    },
  ];
  ...
  return (
    <header
      className={`fixed top-0 w-full z-50 transition-all duration-300 ${
        isScrolled ? "bg-white/95 text-gray-800 backdrop-blur-md shadow-lg" : "bg-transparent text-white"
      }`}
    >
          ...
          <div className="md:flex md:items-center md:gap-12">
            <nav aria-label="Global" className="hidden md:block">
              <ul className="flex items-center gap-10 text-lg">
                ...
                <li className="relative" ref={productMenuRef}>
                          ...
                          {productCategories.map((category) => (
                            <div key={category.id} className="relative">
                              <div
                                ...
                                className={`w-full text-left px-4 py-2.5 text-base text-gray-700 hover:bg-gray-50 hover:text-green-500 rounded-md transition-colors flex items-center justify-between cursor-pointer group ${
                                  activeCategory === category.id ? "bg-gray-50 text-green-500" : ""
                                }`}
                              >
                                <span>{category.name}</span>
                                <svg
                                  className={`w-4 h-4 transition-transform duration-200 text-gray-400 group-hover:text-green-500 ${
                                    activeCategory === category.id ? "rotate-180 text-green-500" : ""
                                  }`}
                                  fill="none"
                                  stroke="currentColor"
                                  viewBox="0 0 24 24"
                              ...
                              <div
                                ...
                                className={`overflow-hidden transition-all duration-300 ease-in-out ${
                                  activeCategory === category.id ? "max-h-60 opacity-100" : "max-h-0 opacity-0"
                                }`}
                              >
          ...
          >
            <div className="pt-4 pb-6 space-y-3 px-4">
              ...
              <div className="space-y-2">
                ...
                <div
                  className={`overflow-hidden transition-all duration-300 ease-in-out ${
                    isMobileProductMenuOpen ? "max-h-[500px] opacity-100" : "max-h-0 opacity-0"
                  }`}
                >
                  <div className="pl-4 space-y-2 mt-1">
                    {productCategories.map((category) => (
                      <div key={category.id} className="space-y-1">
                        <div
                          className={`w-full text-left text-gray-700 py-2 flex items-center justify-between cursor-pointer group rounded-md hover:bg-gray-50 px-2 ${
                            mobileActiveCategory === category.id ? "bg-gray-50 text-green-500" : ""
                          }`}
                          onClick={() =>
                            setMobileActiveCategory(mobileActiveCategory === category.id ? null : category.id)
                          }
                        >
                          <span>{category.name}</span>
                          <svg
                            className={`w-4 h-4 transition-transform duration-200 text-gray-400 group-hover:text-green-500 ${
                              mobileActiveCategory === category.id ? "rotate-180 text-green-500" : ""
                            }`}
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                        ...
                        <div
                          ...
                          className={`overflow-hidden transition-all duration-300 ease-in-out ${
                            mobileActiveCategory === category.id ? "max-h-60 opacity-100" : "max-h-0 opacity-0"
                          }`}
                        >
